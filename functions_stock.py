...
def load_stock(stock_code, start_date, end_date, output_file, source='yf'):
    ...
    if output_file in ['', None]:
        output_file = stock_code.replace('.','')+'_'+start_date.replace('-','')+'_'+end_date.replace('-','')+'.pkl'
    #首先让程序尝试读取已下载并保存的文件
    try:
        df = pd.read_pickle('./data/'+output_file)
    ...
    #如果没有找到文件，则重新进行下载
        ...
        #下载源为yahoo
        if source == 'ts':
            df = get_stock_data_ts(stock_code, start_date, end_date)
        else:
...
def load_stock_data(stock_code, start_date, end_date, output_file, source='yf'):
    ...
    """
    if output_file in ['', None]:
        output_file = stock_code.replace('.','')+'_'+start_date.replace('-','')+'_'+end_date.replace('-','')+'.csv'
    #首先让程序尝试读取已下载并保存的文件
    try:
        df = pd.read_pickle('./data/'+output_file)
        #如果文件已存在，则打印载入股票数据文件完毕
...
# 使用tushare获取股票数据
def get_stock_data_ts(stock_code, start_date, end_date):
    """
    使用tushare获取股票数据
...
def set_stock_excol(stock_data,col_name):
    ...
    stock_data['Log_Rets'] = np.log(1+stock_data['returns'])
