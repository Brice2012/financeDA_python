...
def port_ret_vol_random(rets):
    ...
    print(f'\n预期收益: {pret:.4f}')  #预期收益
    pvar = np.dot(weights.T, np.dot(rets.cov() * 252, weights))  #预期投资组合方差
    print(f'\n预期投资组合方差: {pvar:.4f}')
    pvol = np.sqrt(np.dot(weights.T, np.dot(rets.cov() * 252, weights))) #预期波动率（投资组合标准差）
    print(f'\n预期波动率(投资组合标准差): {pvol:.4f}')  #预期波动率（投资组合标准差）
    # 将weights, pret, pvar, pvol组合成一个二维数组，以便后续使用
    results = np.array([weights, pret, pvar, pvol]).T
    return results

def port_ret_vol_montecarlo(rets, noa, num_simulations=2500):
    """
    随机模拟num_simulations次的预期收益和预期波动率
    """
    ###蒙特卡洛模拟。预期的投资组合收益和方差
    prets = [] # 预期收益
    pvols = [] # 预期波动率
...
def po_efficient_frontier(prets,pvols,trets=None):
    ...
    opts = opts_max_sharpe() # 最大夏普指数情况下的最优投资组合
    optv = opts_min_vol()   # 最小波动率情况下的最优投资组合
...
def po_cap_market_line(prets,pvols):
    import scipy.interpolate as sci
    trets, tvols = po_efficient_frontier(prets,pvols)
